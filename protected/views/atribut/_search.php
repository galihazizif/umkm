<?php
/* @var $this AtributController */
/* @var $model Atribut */
/* @var $form CActiveForm */
?>

<div class="wide form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'action'=>Yii::app()->createUrl($this->route),
	'method'=>'get',
)); ?>

	<div class="row">
		<?php echo $form->label($model,'at_id'); ?>
		<?php echo $form->textField($model,'at_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'at_kategori_id'); ?>
		<?php echo $form->textField($model,'at_kategori_id',array('size'=>5,'maxlength'=>5)); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'at_umkm_id'); ?>
		<?php echo $form->textField($model,'at_umkm_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'at_text'); ?>
		<?php echo $form->textField($model,'at_text',array('size'=>45,'maxlength'=>45)); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton('Search'); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- search-form -->